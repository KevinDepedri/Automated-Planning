...
public:
  ...
  FillBoxWithSupplyAction() : plansys2::ActionExecutorClient("move_1_2_3_4_loaded_carrier", 200ms){
    progress_ = 0.0;
  }
...
private:
  ...
  void do_work()
  {
    if (progress_ < 1.0) {
      ...
      send_feedback(progress_, "Moving 1-2-3-4 loaded carrier in progress");
    } else {
      // Notify that the action is finished using finish. Then moves the node into inactive state
      finish(true, 1.0, "Moving 1-2-3-4 loaded carrier completed");
    ...
    std::cout << "Moving 1-2-3-4 loaded carrier ... [" << std::min(100.0, progress_ * 100.0) << "%]  " <<
    std::flush;
  }
...
int main(int argc, char ** argv)
{
  ...
  node->set_parameter(rclcpp::Parameter("action_name", "move_1_2_3_4_loaded_carrier"));
