...
public:
  ...
  FillBoxWithSupplyAction() : plansys2::ActionExecutorClient("unfill_box_with_supply", 200ms){
    progress_ = 0.0;
  }
...
private:
  ...
  void do_work()
  {
    if (progress_ < 1.0) {
      ...
      send_feedback(progress_, "Unfilling box in progress");
    } else {
      // Notify that the action is finished using finish. Then moves the node into inactive state
      finish(true, 1.0, "Unfilling box completed");
    ...
    std::cout << "Unfilling ... [" << std::min(100.0, progress_ * 100.0) << "%]  " <<
    std::flush;
  }
...
int main(int argc, char ** argv)
{
  ...
  node->set_parameter(rclcpp::Parameter("action_name", "unfill_box_with_supply"));
