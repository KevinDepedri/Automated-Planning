...
public:
  ...
  FillBoxWithSupplyAction() : plansys2::ActionExecutorClient("unload_carrier_position_4", 200ms){
    progress_ = 0.0;
  }
...
private:
  ...
  void do_work()
  {
    if (progress_ < 1.0) {
      ...
      send_feedback(progress_, "Unloading box on carrier in position 4 in progress");
    } else {
      // Notify that the action is finished using finish. Then moves the node into inactive state
      finish(true, 1.0, "Unloading box on carrier in position 4 completed");
    ...
    std::cout << "Unloading box on carrier in position 4 ... [" << std::min(100.0, progress_ * 100.0) << "%]  " <<
    std::flush;
  }
...
int main(int argc, char ** argv)
{
  ...
  node->set_parameter(rclcpp::Parameter("action_name", "unload_carrier_position_4"));
