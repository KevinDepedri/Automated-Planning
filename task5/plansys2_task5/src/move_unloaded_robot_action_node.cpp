...
public:
  ...
  FillBoxWithSupplyAction() : plansys2::ActionExecutorClient("move_unloaded_robot", 200ms){
    progress_ = 0.0;
  }
...
private:
  ...
  void do_work()
  {
    if (progress_ < 1.0) {
      ...
      send_feedback(progress_, "Moving unloaded robot in progress");
    } else {
      // Notify that the action is finished using finish. Then moves the node into inactive state
      finish(true, 1.0, "Moving unloaded robot completed");
    ...
    std::cout << "Moving unloaded robot ... [" << std::min(100.0, progress_ * 100.0) << "%]  " <<
    std::flush;
  }
...
int main(int argc, char ** argv)
{
  ...
  node->set_parameter(rclcpp::Parameter("action_name", "move_unloaded_robot"));
