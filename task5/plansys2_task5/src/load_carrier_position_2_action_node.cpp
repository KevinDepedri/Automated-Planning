...
public:
  ...
  FillBoxWithSupplyAction() : plansys2::ActionExecutorClient("load_carrier_position_2", 200ms){
    progress_ = 0.0;
  }
...
private:
  ...
  void do_work()
  {
    if (progress_ < 1.0) {
      ...
      send_feedback(progress_, "Loading box on carrier in position 2 in progress");
    } else {
      // Notify that the action is finished using finish. Then moves the node into inactive state
      finish(true, 1.0, "Loading box on carrier in position 2 completed");
    ...
    std::cout << "Loading box on carrier in position 2 ... [" << std::min(100.0, progress_ * 100.0) << "%]  " <<
    std::flush;
  }
...
int main(int argc, char ** argv)
{
  ...
  node->set_parameter(rclcpp::Parameter("action_name", "load_carrier_position_2"));
