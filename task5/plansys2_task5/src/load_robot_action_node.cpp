...
public:
  ...
  FillBoxWithSupplyAction() : plansys2::ActionExecutorClient("load_robot", 200ms){
    progress_ = 0.0;
  }
...
private:
  ...
  void do_work()
  {
    if (progress_ < 1.0) {
      ...
      send_feedback(progress_, "Loading box on robot in progress");
    } else {
      // Notify that the action is finished using finish. Then moves the node into inactive state
      finish(true, 1.0, "Loading box on robot completed");
    ...
    std::cout << "Loading box on robot ... [" << std::min(100.0, progress_ * 100.0) << "%]  " <<
    std::flush;
  }
